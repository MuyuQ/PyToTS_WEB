import { getBookmarks, getProgress } from '../lib/progress-store';
...
export function renderBookmarks() {
  ...
  const lessonMap = new Map(progress.lessons.map(l => [l.path, l]));
  ...
  function getCategory(path: string): string {
    if (path.startsWith('/paths/foundation')) return '基础语法';
    if (path.startsWith('/paths/migration')) return '迁移指南';
    if (path.startsWith('/paths/advanced')) return '高级特性';
    if (path.startsWith('/algorithms')) return '算法';
    return '其他';
  }

  // 按类别分组
  const grouped: Record<string, string[]> = bookmarks.reduce(function(acc: Record<string, string[]>, path: string) {
  ...
  for (const category in grouped) {
    ...
    for (const path of paths) {
      const title = (titleMap as Record<string, string>)[path] || path;
      const isCompleted = lessonMap.has(path);
