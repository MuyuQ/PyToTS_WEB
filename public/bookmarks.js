...
(function () {
  // 子路径部署的 base 前缀：从自身 script 地址推导
  // （MDX 会改写内联 script 体，不能用内联脚本传参）
  var BASE = (function () {
    try {
      ...
      if (src) {
        ...
        if (path.slice(-13) === "/bookmarks.js") return path.slice(0, -13);
      }
    } catch (e) {}
    return "";
  })();

  // 归一化：剥离 base 前缀、去掉末尾斜杠，与 titleMap 的键保持一致
  function norm(path) {
  ...
  function renderBookmarks() {
    ...
    if (bookmarks.length === 0) {
      container.innerHTML =
        '<p class="empty-state">暂无书签，在课程和算法页面点击「收藏」按钮添加。</p>';
      return;
    }
    ...
    for (var category in grouped) {
      ...
      for (var j = 0; j < paths.length; j++) {
        ...
        html += '<a href="' + BASE + path + '/" class="bookmark-card">';
